package com.bdpolice.kms.ui.repository.logistics
...
class Logistics @Inject constructor(
    private val api: API
) {

    private val mutableStateFlow = MutableStateFlow<NetworkResult<LogisticsResponse>>(NetworkResult.Empty())
    val stateFlow : StateFlow<NetworkResult<LogisticsResponse>> get() = mutableStateFlow

    suspend fun getLogistics(token : String, employeeCode : String){
        try {
            mutableStateFlow.emit(NetworkResult.Loading())
            val response = api.getLogistics(
                tokenData = token,
